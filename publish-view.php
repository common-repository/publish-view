 ...
 * Author URI: https://launchinteractive.com.au
 * License: GPL2
*/
/*
	...
	public function submitbox_start(){
		...
		if($type->public) {
			...
			if($post->post_status == 'auto-draft' || $post->post_status == 'draft') {
				$options = array('onclick'=>"if(jQuery('#publish').is(':disabled')){return false};jQuery('#pv_view_hidden').remove();jQuery(this).after('<input id=\"pv_view_hidden\" type=\"hidden\" name=\"pv_view\" value=\"Y\" />')",'title'=>'Publish & View','id'=>'pv_publish');
			} else if($post->post_status == 'publish') {
				$options = array('onclick'=>"jQuery('#pv_view_hidden').remove();jQuery(this).after('<input id=\"pv_view_hidden\" type=\"hidden\" name=\"pv_view\" value=\"Y\" />')",'title'=>'Update & View','id'=>'pv_publish');
			}
			...
			if ( 'publish' != $post->post_status && 'future' != $post->post_status && 'pending' != $post->post_status ) {
				submit_button('&#xf177;','button','save',false, array('id'=>'pv_draft','Save Draft & View','onclick'=>"jQuery('#pv_view_hidden').remove();jQuery(this).after('<input id=\"pv_view_hidden\" type=\"hidden\" name=\"pv_view\" value=\"Y\" />')"));
				
				$onload .= 'jQuery(\'#save-post\').wrap(\'<div class="pv_wrap"></div>\').parent().append(jQuery(\'#pv_draft\'));';
			}	
			...
			if($previewNewWindow == 'disable') {
				...
				echo 'jQuery(\'#post-preview\').attr(\'target\',\'_self\');jQuery(\'#message.updated a,#view-post-btn a\').removeAttr(\'target\');';
			}
			
			echo 'jQuery(function(){'.$onload.'jQuery(\'#publish\').wrap(\'<div class="pv_wrap"></div>\').parent().append(jQuery(\'#pv_publish\'));});</script>';
